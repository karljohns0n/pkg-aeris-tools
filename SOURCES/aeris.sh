...
mysql-add() {
	if [[ $# -ne 2 ]]; then
		...
		if [[ ! $SQLDB =~ ^[a-zA-Z0-9_]+$ ]]; then
			echo "Error: Invalid database name. Use only alphanumeric characters and underscores." >&2
			return 1
		fi
		
		# Validate username (alphanumeric and underscore only)
		if [[ ! $SQLUSER =~ ^[a-zA-Z0-9_]+$ ]]; then
			echo "Error: Invalid username. Use only alphanumeric characters and underscores." >&2
			return 1
		fi
		
		SQLPASS=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13)

		$SQL_BIN -e "CREATE DATABASE \`$SQLDB\`;" || { echo "Failed to create database" >&2; return 1; }
		$SQL_BIN -e "CREATE USER IF NOT EXISTS '$SQLUSER'@'localhost' IDENTIFIED BY '$SQLPASS';" || { echo "Failed to create user" >&2; return 1; }
		$SQL_BIN -e "GRANT ALL PRIVILEGES ON \`$SQLDB\`.* TO '$SQLUSER'@'localhost';" || { echo "Failed to grant privileges" >&2; return 1; }
		$SQL_BIN -e "FLUSH PRIVILEGES;" || { echo "Failed to flush privileges" >&2; return 1; }
...
mysql-optimize() {
	...
	if [[ -f /root/.my.cnf ]]; then
		...
		"$SQLCHECK_BIN" -u root -p --auto-repair --optimize --all-databases
	fi
}
